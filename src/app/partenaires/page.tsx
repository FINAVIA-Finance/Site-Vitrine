...
import {INTEGRATION_BLOCKS, INTEGRATION_STEPS, TECH_PARTNERS_LIST, TRANSFORMATION_OBJECTIVES} from '@/lib/data';
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="relative bg-[#07036e] text-white pt-48 pb-24 px-6 overflow-hidden rounded-b-[3rem]">
        ...
        <div
          className="absolute inset-0 bg-gradient-to-r from-[#07036e] via-[#07036e]/90 to-transparent z-0 pointer-events-none"></div>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <motion.div initial={{opacity: 0, x: -50}} animate={{opacity: 1, x: 0}} transition={{duration: 0.8}}
            ...
            <p className="text-xl text-white/80 leading-relaxed font-light mb-8 max-w-xl">
              Nous sélectionnons et intégrons les meilleures solutions du marché (comptabilité, trésorerie,
              automatisation, IA) pour automatiser
              vos processus et fiabiliser votre pilotage.
            </p>
      ...
      <section className="py-24 px-6 bg-white relative">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="text-center mb-16">
            ...
            <p className="text-gray-500 mt-4 max-w-2xl mx-auto">Définissez les bases d'une fonction finance agile et
              efficace, avec des outils intelligents qui facilitent le pilotage et l'exécution au quotidien.</p>
          </div>
          <div className="grid md:grid-cols-2 gap-8">
            {TRANSFORMATION_OBJECTIVES.map((obj, idx) => (
              <motion.div key={idx} whileHover={{y: -5}}
                          className="bg-white p-8 rounded-3xl border border-gray-100 shadow-xl group transition-all">
                <div
                  className="w-12 h-12 bg-[#e51990]/10 rounded-xl flex items-center justify-center text-[#e51990] mb-6">
                  <obj.icon size={24}/>
                </div>
                <h3 className="text-xl font-bold text-[#07036e] mb-4">{obj.title}</h3>
                <p className="text-gray-600 mb-6 text-sm">{obj.desc}</p>
                <div
                  className="flex flex-wrap gap-4 items-center opacity-70 grayscale group-hover:grayscale-0 transition-all">
                  {obj.logos.map((logo, lIdx) => (
                    <div key={lIdx} className="relative h-8 w-[120px]">
                      <Image src={logo} alt="Partner" fill
                             className={logo.src.includes('yooz') ? 'object-cover' : 'object-contain'}/>
                    </div>
                  ))}
                </div>
              </motion.div>
          ...
          {INTEGRATION_BLOCKS.map((block, idx) => (
            <div key={idx} className="grid md:grid-cols-2 gap-12 items-center">
              <div className={`${idx % 2 !== 0 ? 'md:order-2' : ''}`}>
                <span
                  className="text-[#e51990] font-bold text-sm uppercase tracking-wider mb-2 block">{block.id}. {block.tag}</span>
                <h3 className="text-3xl font-bold text-[#07036e] mb-6">{block.title}</h3>
                <div className="space-y-6">
                  <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm">
                    <h4 className="font-bold text-[#07036e] mb-2 flex items-center gap-2">
                      <Settings size={18} className="text-[#e51990]"/> Actions Clés
                    </h4>
                    <p className="text-sm text-gray-600">{block.actions}</p>
                  </div>
                  <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm">
                    <h4 className="font-bold text-[#07036e] mb-2 flex items-center gap-2"><CheckCircle2 size={18}
                                                                                                        className="text-[#e51990]"/> Bénéfices
                    </h4>
              ...
              <div
                className={`h-80 rounded-3xl overflow-hidden shadow-2xl relative ${idx % 2 !== 0 ? 'md:order-1' : ''}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-8">
            {INTEGRATION_STEPS.map((item, idx) => (
              <motion.div key={idx} initial={{opacity: 0, y: 20}} whileInView={{opacity: 1, y: 0}}
                          ...
                          className="relative z-10 bg-white/10 backdrop-blur-md p-6 rounded-2xl border border-white/20 text-center group hover:-translate-y-2 transition-all">
                <div
                  className="w-10 h-10 mx-auto bg-[#e51990] text-white rounded-full flex items-center justify-center font-bold mb-4 shadow-md group-hover:scale-110">
                  {item.step}
                </div>
            ...
            {TECH_PARTNERS_LIST.map((cat, idx) => (
              <div key={idx} className="border border-gray-100 rounded-3xl p-8 hover:shadow-lg transition-all bg-white">
                <h3 className="font-bold text-[#07036e] mb-6 flex items-center gap-2">
                  <cat.icon size={20} className="text-[#e51990]"/>
                  {cat.category}
                </h3>
                <div className="space-y-6">
