...
const content = LEGAL_PAGE_CONTENT['rgpd'];
                                    ...
                                    {section.content.map((item, pIndex) => {
                                        if (item.includes('http')) {
                                            ...
                                            if (url) {
                                                return (
                                                    <p key={pIndex}>
                                                        {item.replace(url, '').trim()}{' '}
                                                        <a href={url} target="_blank" rel="noopener noreferrer" className="text-[#e51990] font-bold hover:underline">
                                                            {url}
                                                        </a>
                                                    </p>
                                                );
                                        ...
                                        if (item.includes('@')) {
                                            const email = item.match(/[\w.-]+@[\w.-]+/)?.[0];
                                            if (email) {
                                                return (
                                                    <p key={pIndex}>
                                                        {item.replace(email, '').trim()}{' '}
